 ...
 *
 * <p>Ningun nivel de la jerarquia tiene estado: direcciones, coordinaciones y
 * laboratorios no se desactivan, solo se crean y se corrigen sus datos.</p>
 */
export abstract class OrganizacionPuerto {
  /** Arbol completo de la institucion (RF-09, RF-15). Solo Administrador. */
  abstract estructura(): Observable<Estructura>;

  /**
   * RF-10: las direcciones son las dos de la institucion y se precargan con el
   * esquema. No se crean desde la aplicacion; solo se leen y se corrigen sus
   * datos.
   */
  abstract listarDirecciones(): Observable<Direccion[]>;
  abstract editarDireccion(id: number, peticion: DireccionPeticion): Observable<Direccion>;

  abstract listarCoordinaciones(direccionId?: number | null): Observable<Coordinacion[]>;
  ...
  abstract listarLaboratorios(coordinacionId: number): Observable<Laboratorio[]>;
